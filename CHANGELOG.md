...
* The task is a self-contained launcher (`_clearml_launcher.py`) that runs the
  ...
  are not touched.
* Task title is `us_<command>_<run folder>`; while it runs, the launcher
  follows cg-us's `[run|extend|analyze] <system> rep<N>` lines and updates the
  title (`us_<command>_<run folder>_<system>_rep<N>`) and comment, so the queue and
